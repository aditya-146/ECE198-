...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  if (moisture_value <= 850) {
	      ...
	      if (last_pwm_value != 850) {
	          for (uint16_t pwm_value = last_pwm_value; pwm_value >= 414; pwm_value--) {
	              __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_2, pwm_value);
	              HAL_Delay(0.5); // Adjust delay for smooth movement
	          }
	          last_pwm_value = 414; // Update the position
	      }
	      ...
	      if (last_pwm_value != 1250) {
	          for (uint16_t pwm_value = last_pwm_value; pwm_value <= 1250; pwm_value++) {
	              __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_2, pwm_value);
	              HAL_Delay(1.5); // Adjust delay for smooth movement
	          }
