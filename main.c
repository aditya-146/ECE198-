...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  moisture_value = atoi(received_data);  // 'atoi' converts string to integer

	      // Clear the second line of the LCD to prevent overlap of old values
	  ...
	  if (moisture_value >= 0 && moisture_value <= 1000) {
	          ...
	          HD44780_PrintStr("Invalid Value");  // In case the value is outside expected range
	  }
